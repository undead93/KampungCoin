...
#if defined(NDEBUG)
# error "Kampungcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x513c34193c460889399043d688a2a10cb65fa7615ffbc58ddf7985b353ed2666");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kampungcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kampungcoin Signed Message:\n";
...
{
    // Kampungcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Kampungcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Kampungcoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Kampungcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kampungcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kampungcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1a617b89867d88bc5f7b137c3d996eedae97befd9ba21f3c52485472f7251511");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "test kampung coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04127fd2c8d290806917d566429bad61f7af76f1ae3550f3cc38b7f733fdf1176a3c4bb750bfad99ba26aaad3a94d4c387959d72034684133672f43bdb433051a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517223896;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085825131;

        if (fTestNet)
        {
            block.nTime    = 1517223813;
            block.nNonce   = 389387063;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xa92d51939ddf0ebb54e07524899e7bdb50ed5c0f4f469213ee9371ad2d566527"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Kampungcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KampungcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KampungcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KampungcoinMiner(CWallet *pwallet)
{
    printf("KampungcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kampungcoin-miner");
    ...
    try { loop {
        ...
        printf("Running KampungcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KampungcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KampungcoinMiner, pwallet));
}
